(function ($) {
	...
	class Silence {
		...
		buildCustomElements() {
			...
			$title.html($title.html().replace(`袅袅炊烟 | ${autherName}`, `${blogTitle}`));
		...
		buildBloggerProfile() {
			...
			var title = '<div class="site-branding">' +
					...
					'<a href="https://www.cnblogs.com/hfjh/" alt="袅袅炊烟">' +
					'<ruby><span class="sakuraso">hfjh</span><span class="no"></span><span class="shironeko"></span>' +
					'<rt class="chinese-font">青山白云 袅袅炊烟</rt></ruby></a></span>' +
					'</div>'
			$('body').prepend(title);
		...
		setHomeSuiBiList() {
			...
			for (let i = article_list.length - 1; i >= 0; i--) {
				...
				let desc = $('.day').find('div.postDesc')[i].textContent;//posted @ 2019-07-16 13:27  阅读 (3073) 评论 (56)<a href="https://i.cnblogs.com/EditPosts.aspx?postid=11194299" rel="nofollow">编辑</a>
		...
		postHeader() {
			...
			if (window.location.href.indexOf('articles') === -1) {
				var header =
						`<p class="entry-census"><span><a href="https://www.cnblogs.com/hfjh/"><img src="https://pic.cnblogs.com/avatar/3077771/20230114110511.png"></a></span><span><a href="https://www.cnblogs.com/hfjh/">Toretto</a></span><span class="bull">·</span>${post_date}<span class="bull">·</span>${post_view_count} 次阅读</p>`;
